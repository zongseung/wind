...
- **`submission_v5.csv`** — 시드 5개 앙상블. 홀드아웃 점수는 v4와 동률이지만 **시드 선택 운이 제거된 견고한 버전** → 2025 실전 기대값 우위. 1-NMAE 0.8720, FICR 0.4203.
- `submission_v4.csv` — random search가 고른 단일 시드(15). CV 점수에 시드 운이 포함된 낙관치.

참고: 리더보드(2026-07-08 기준) 1등 총점 0.66524, 20~50위가 0.639~0.643에 밀집. 상위권 1-NMAE는 ~0.87에서 포화 상태라 **순위는 FICR에서 갈린다**.
